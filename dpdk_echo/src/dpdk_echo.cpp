...
#define CMD_OPT_NB_CYCLES "nb-cycles"
...
static const struct option long_options[] = {
    ...
    {CMD_OPT_NB_CYCLES, required_argument, NULL, CMD_OPT_NB_CYCLES_NUM},
  ...
  while ((opt = getopt_long(argc, argv, short_options, long_options,
    ...
    switch (opt) {
      case CMD_OPT_HELP_NUM:
        return 1;
      case CMD_OPT_NB_CYCLES_NUM:
        parsed_args->nb_cycles = atoi(optarg);
        break;
...
static int lcore_work(void* arg) {
  ...
  const uint32_t nb_queues = q_per_core;
  const uint32_t my_nb_cycles = nb_cycles;
  ...
  while (!quit) {
    for (uint32_t q_offset = 0; q_offset < nb_queues; ++q_offset) {
      ...
      for (uint16_t i = 0; i < nb_rx; ++i) {
        uint8_t* buf = rte_pktmbuf_mtod_offset(bufs[i], uint8_t*, 60);
        ++(*buf);
        for (uint32_t j = 0; j < my_nb_cycles; ++j) {
          asm("nop");
        }
      }
...
int main(int argc, char* argv[]) {
  ...
  nb_cycles = parsed_args.nb_cycles;
