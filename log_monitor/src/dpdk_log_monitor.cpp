...
#define CMD_OPT_NB_STREAMS "nb-streams"
enum {
...
static void print_usage(const char* program_name) {
  printf(
      "%s [EAL options] -- REGEX_FILE"
      " [--help] |\n"
      " [--q-per-core]\n"
      " [--nb-streams]\n\n"

      "  REGEX_FILE: File containing regular expressions to match against\n"
      "  --help: Show this help and exit\n"
      "  --q-per-core: Number of queues per core\n"
      "  --nb-streams: Number of streams\n",
      program_name);
}
...
static const struct option long_options[] = {
    ...
    {CMD_OPT_NB_STREAMS, required_argument, NULL, CMD_OPT_NB_STREAMS_NUM},
    {0, 0, 0, 0}};
...
struct parsed_args_t global_parsed_args;
...
static int parse_args(int argc, char** argv,
  ...
  while ((opt = getopt_long(argc, argv, short_options, long_options,
    ...
    switch (opt) {
      ...
      case CMD_OPT_Q_PER_CORE_NUM:
        ...
        break;
      case CMD_OPT_NB_STREAMS_NUM:
        parsed_args->nb_streams = atoi(optarg);
        break;
      default:
        return -1;
    }
  ...
  parsed_args->regex_filename = argv[optind++];
...
static int lcore_work(void* arg) {
  ...
  uint32_t nb_queues = global_parsed_args.q_per_core;
  uint32_t nb_streams = global_parsed_args.nb_streams;

  LogMonitor log_monitor(global_parsed_args.regex_filename, nb_streams);
  int ret = log_monitor.setup();
  if (ret) {
    rte_exit(EXIT_FAILURE, "Issue setting up log monitor : \"%s\"\n",
             rte_strerror(ret));
  }
  ...
  unsigned lcore_idx = first_queue / nb_queues;
  ...
  while (!quit) {
    for (uint32_t q_offset = 0; q_offset < nb_queues; ++q_offset) {
      ...
      for (uint16_t i = 0; i < nb_rx; ++i) {
        ...
        nb_matches += log_monitor.lookup(buf, len, q_offset);
      }
...
int main(int argc, char* argv[]) {
  ...
  ret = parse_args(argc, argv, &global_parsed_args);
  if (ret) {
    print_usage(argv[0]);
    if (ret == 1) {
      return 0;
  ...
  uint32_t q_per_core = global_parsed_args.q_per_core;

  assert(global_parsed_args.nb_streams == q_per_core);
