...
struct BasicDatagramClientConfig {
  // The network type.
  #ifdef USE_ENSO
  typedef ::mica::network::Enso Network;
  #else
...
class DatagramClient {
 public:
  ...
  void probe_reachability(volatile bool& quit);
 ...
 private:
  ...
  struct ThreadState {
    #ifdef USE_ENSO
    std::unique_ptr<enso::Device> device;
    #endif // USE_ENSO
  ...
  #ifdef USE_ENSO
  void probe_send(RXTXState& rx_tx_state, ServerEndpointInfo& sei);
  bool probe_receive(RXTXState& rx_tx_state, ServerEndpointInfo& sei,
                     volatile bool& quit);
  #else
  ...
  #ifdef USE_ENSO
  void make_new_pending_request_batch(RXTXState& rx_tx_state,
  ...
  #ifdef USE_ENSO
  void flush_pending_request_batch(RXTXState& rx_tx_state,
