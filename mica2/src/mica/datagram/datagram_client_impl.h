...
template <class StaticConfig>
void DatagramClient<StaticConfig>::probe_reachability(volatile bool& quit) {
  ...
  for (auto eid : network_->get_endpoints()) {
    ...
    if (ei.owner_lcore_id == lcore_id) { my_eids.push_back(eid); }
  }

  // Set the endpoint count and initialize state
  thread_state.endpoint_count = my_eids.size();
  for (size_t idx = 0; idx < my_eids.size(); idx++) {
    thread_state.rx_tx_states[idx].eid = my_eids[idx];
    thread_state.rx_tx_states[idx].pending_tx.count = 0;
  }

  #ifdef USE_ENSO
  auto device = enso::Device::Create(my_eids.size());
  for (size_t idx = 0; idx < my_eids.size(); idx++) {
    const auto& ei = network_->get_endpoint_info(my_eids[idx]); 

    // Configure RX
    auto rx_pipe = device->AllocateRxPipe();
    assert(rx_pipe != nullptr); // Sanity check
    thread_state.rx_tx_states[idx].rx_pipe = rx_pipe;
    rx_pipe->set_context(&(thread_state.rx_tx_states[idx]));
    rx_pipe->Bind(ei.udp_port, 0, ei.ipv4_addr, 0, IPPROTO_UDP);

    // Configure TX
    auto tx_pipe = device->AllocateTxPipe();
    assert(tx_pipe != nullptr); // Sanity check
    thread_state.rx_tx_states[idx].tx_pipe = tx_pipe;

    // Allocate TX state
    uint8_t* buffer = tx_pipe->AllocateBuf(
      1536 /** ROUNDED_ETHER_MAX_LEN */ *
      StaticConfig::kTXBurst);

    thread_state.rx_tx_states[idx].pending_tx.start_tx_buffer = buffer;
    thread_state.rx_tx_states[idx].pending_tx.current_tx_buffer = buffer;
  }
  thread_state.device = std::move(device);
  #endif // USE_ENSO
    ...
    for (uint16_t remote_eindex = 0; remote_eindex < s.endpoint_count;
      ...
      while (stopwatch_.diff_in_cycles(stopwatch_.now(), start) <
        ...
        for (uint16_t eindex = 0; eindex < thread_state.endpoint_count;
          ...
          auto& rx_tx_state = thread_state.rx_tx_states[eindex];
          #ifdef USE_ENSO
          if (probe_receive(rx_tx_state, sei, quit))
            valid_local_eindices.push_back(eindex);
          #else
          if (probe_receive(rx_tx_state.eid, sei, quit))
            valid_local_eindices.push_back(eindex);
          #endif // USE_ENSO
        }
...
#ifdef USE_ENSO
template <class StaticConfig>
void DatagramClient<StaticConfig>::probe_send(RXTXState& rx_tx_state,
                                              ServerEndpointInfo& sei) {

  RequestBatchBuilder<PacketBuffer> b(
    rx_tx_state.pending_tx.current_tx_buffer);

  b.set_src_mac_addr(network_->get_endpoint_info(rx_tx_state.eid).mac_addr);
  ...
  // Advance the current TX buffer pointer
  rx_tx_state.pending_tx.current_tx_buffer = (
    enso::get_next_pkt(rx_tx_state.pending_tx.current_tx_buffer));
  ...
  // Reallocate a sufficiently large TX buffer
  uint8_t* buffer = rx_tx_state.tx_pipe->AllocateBuf(
    1536 /** ROUNDED_ETHER_MAX_LEN */ *
    StaticConfig::kTXBurst);

  rx_tx_state.pending_tx.current_tx_buffer = buffer;
  rx_tx_state.pending_tx.start_tx_buffer = buffer;
}
#else
template <class StaticConfig>
...
#ifdef USE_ENSO
template <class StaticConfig>
bool DatagramClient<StaticConfig>::probe_receive(RXTXState& rx_tx_state,
                                                 ServerEndpointInfo& sei,
                                                 volatile bool& quit) {
  ...
  while (!quit) {
    auto batch = rx_tx_state.rx_pipe->RecvPkts(StaticConfig::kRXBurst);
    for (auto pkt : batch) {
      RequestBatchReader<PacketBuffer> r(pkt);

      if (r.is_valid() && r.is_response()) {
        if (r.find_next() && r.get_opaque() == opaque) {
          connected = true;
        }
      }
    }
    rx_tx_state.rx_pipe->Clear();
    if (connected) break;
  }
...
DatagramClient<StaticConfig>::append_request(
  ...
  // TODO(natre): Relax assumption.
  assert(!spread_request);
  ...
  make_new_pending_request_batch(rx_tx_state, *prb);
  #endif // USE_ENSO
  ...
  if (prb->b.get_count() >= StaticConfig::kMaxRequestBatchSize) {
    ...
    #ifdef USE_ENSO
    flush_pending_request_batch(rx_tx_state, server_index, *prb, now);
    #else
...
void DatagramClient<StaticConfig>::make_new_pending_request_batch(
    RXTXState& rx_tx_state, PendingRequestBatch& prb) {
  if (StaticConfig::kVerbose) printf("make_new_pending_request_batch()\n");
  prb.b = RequestBatchBuilder<uint8_t>(rx_tx_state.pending_tx.current_tx_buffer);
}
#else
template <class StaticConfig>
void DatagramClient<StaticConfig>::make_new_pending_request_batch(
...
void DatagramClient<StaticConfig>::flush_pending_request_batch(
    RXTXState& rx_tx_state, uint32_t server_index,
