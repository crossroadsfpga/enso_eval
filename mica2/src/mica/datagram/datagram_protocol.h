...
class RequestBatchReader {
 public:
  ...
  explicit RequestBatchReader(const T* buf)
      : buf_(buf), total_length_(0), h_(nullptr) {}

  const char* get_data() const {
    if constexpr (std::is_same<T, uint8_t>::value) { return (const char*) buf_; }
    else { return buf_->get_data(); }
  }

  bool is_valid() {
    auto mac = reinterpret_cast<const rte_ether_hdr*>(this->get_data());
    auto bh = reinterpret_cast<const RequestBatchHeader*>(this->get_data());
    auto ip =
        reinterpret_cast<const rte_ipv4_hdr*>(this->get_data() + sizeof(rte_ether_hdr));
    auto udp = reinterpret_cast<const rte_udp_hdr*>(
        this->get_data() + sizeof(rte_ether_hdr) + sizeof(rte_ipv4_hdr));

    #ifndef USE_ENSO
    total_length_ = buf_->get_length();

    if (total_length_ < sizeof(RequestBatchHeader)) {
    ...
    auto ip_length = rte_be_to_cpu_16(ip->total_length);
    #ifdef USE_ENSO
    total_length_ = ip_length + sizeof(rte_ether_hdr);
    #else
    if (ip_length != total_length_ - sizeof(rte_ether_hdr)) {
  ...
  bool find_next() {
    if (h_ == nullptr)
      h_ = reinterpret_cast<const RequestHeader*>(this->get_data() +
    ...
    if (reinterpret_cast<const char*>(h_) + sizeof(RequestHeader) -
            this->get_data() > total_length_)
      return false;
    if (reinterpret_cast<const char*>(h_) + sizeof(RequestHeader) +
            ::mica::util::roundup<8>(get_key_length()) +
            ::mica::util::roundup<8>(get_value_length()) - this->get_data() > total_length_)
      return false;
...
class RequestBatchBuilder {
 public:
  ...
  char* get_data() const {
    if constexpr (std::is_same<T, uint8_t>::value) { return (char*) buf_; }
    else { return buf_->get_data(); }
  }
  ...
  explicit RequestBatchBuilder(T* buf)
      : buf_(buf),
        h_(reinterpret_cast<RequestHeader*>(this->get_data() +
                                            sizeof(RequestBatchHeader))),
  ...
  void set_src_mac_addr(const rte_ether_addr& v) {
    auto mac = reinterpret_cast<rte_ether_hdr*>(this->get_data());
    mac->s_addr = v;
  }
  ...
  void set_src_ipv4_addr(uint32_t v) {
    auto ip = reinterpret_cast<rte_ipv4_hdr*>(this->get_data() + sizeof(rte_ether_hdr));
    ip->src_addr = v;
  }
  ...
  void set_src_udp_port(uint16_t v) const {
    auto udp = reinterpret_cast<rte_udp_hdr*>(this->get_data() + sizeof(rte_ether_hdr) +
                                          sizeof(rte_ipv4_hdr));
    udp->src_port = v;
  }
  ...
  void set_request() {
    auto bh = reinterpret_cast<RequestBatchHeader*>(this->get_data());
    bh->magic = 0x78;
  }
  ...
  void finalize() {
    ...
    size_t packet_length =
        static_cast<size_t>(reinterpret_cast<char*>(h_) - this->get_data());
