...
void heapify(minHeap *hp, int i) {
    int smallest = (LCHILD(i) < hp->size && hp->elem[LCHILD(i)].count < hp->elem[i].count) ? LCHILD(i) : i ;
    if(RCHILD(i) < hp->size && hp->elem[RCHILD(i)].count < hp->elem[smallest].count) {
        smallest = RCHILD(i) ;
    }
    if(smallest != i) {
...
/*
    ...
    node
*/
uint32_t getMaxNode(minHeap *hp, int i) {
    if(LCHILD(i) >= hp->size) {
